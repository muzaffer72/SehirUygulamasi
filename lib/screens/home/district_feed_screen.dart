...
class DistrictFeedScreen extends ConsumerStatefulWidget {
  const DistrictFeedScreen({Key? key}) : super(key: key);

  @override
  ConsumerState<DistrictFeedScreen> createState() => _DistrictFeedScreenState();
}

class _DistrictFeedScreenState extends ConsumerState<DistrictFeedScreen> {
  ...
  void initState() {
    ...
    // İlk yüklemede ilçe filtresiyle gönderileri getir
    WidgetsBinding.instance.addPostFrameCallback((_) {
      final filters = ref.read(postFiltersProvider);
      // İlçe bazlı filtreleme yap - hem şehir hem ilçe belirtilmeli
      if (filters.cityId != null) {
        ref.read(postsProvider.notifier).filterPosts(
          cityId: filters.cityId,
          districtId: filters.districtId, // İlçe filtresi kullan
  ...
  Future<void> _loadMoreData() async {
    ...
    // Load more posts with district filter
    await ref.read(postsProvider.notifier).filterPosts(
      cityId: filters.cityId,
      districtId: filters.districtId, // İlçe filtresi
      categoryId: filters.categoryId,
    );
  ...
  Widget build(BuildContext context) {
    ...
    final districtId = filters.districtId; // Aktif ilçe ID'si
          ...
          Expanded(
            child: RefreshIndicator(
              ...
              child: CustomScrollView(
                ...
                slivers: [
                  // İlçe Başlığı (Seçili ilçe varsa göster)
                  if (districtId != null)
                    SliverToBoxAdapter(
                      child: Padding(
                    ...
                    child: Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          const SurveySlider(filterType: 'district'),
                        ],
                      ),
                    ),
                  ),
  ...
  void _sortPosts(String sortBy) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('İlçe gönderileri "$sortBy" olarak sıralandı')),
    );
  }
      ...
      SnackBar(
        content: Text(
          type == PostType.problem
              ? 'Sadece ilçe şikayetleri gösteriliyor'
              : 'Sadece ilçe önerileri gösteriliyor',
        ),
      ),
    );
  }
